...
func generate_level():
	...
	while room_positions.size() < ROOM_COUNT:
		...
		var new_room = find_valid_neighbor(current_room)
		if new_room != null:
	...
	var boss_position = find_valid_neighbor(boss_hallway)
	...
	if boss_position != null:
		...
		print("error placing exit, please contact Martin, he messed up")
